...
public final class JrtModuleFileSystem extends ModuleFileSystem {
    ...
    static Optional<FileSystem> create(FileSystemProvider provider, URI uri, URI location) throws IOException {
        if (location.getScheme().equals("jrt")) {
            ...
            return Optional.of(new JrtModuleFileSystem(basePath, provider));
        }
    ...
    JrtModuleFileSystem(Path basePath, FileSystemProvider provider) {
